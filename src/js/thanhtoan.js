...
function buy() {
    ...
    for (let i = 0; i < cartArray.length; i++) {
        ...
        if (product?.productName) {
            // Check if there's an existing entry for this product on the same date
            const existingStatIndex = productStats.findIndex(
                stat => stat.productID === product.productID && stat.date === currentDate
            );
            
            if (existingStatIndex !== -1) {
                // Update existing date-specific entry
                productStats[existingStatIndex].quantitySold += product.quantity;
                productStats[existingStatIndex].totalRevenue += Number(product.newPrice) * Number(product.quantity);
            } else {
                // Create new entry for the product on this date
                productStats.push({
                    productID: product.productID,
                    productName: product.productName,
                    quantitySold: product.quantity,
                    totalRevenue: Number(product.newPrice) * Number(product.quantity),
                    date: currentDate
                });
            }
        }
